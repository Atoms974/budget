...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if uploaded:
        ...
        col_s = c4.selectbox("Solde", cols) # Ajout du choix de la colonne solde
        ...
        if st.button("🚀 Lancer l'importation"):
            ...
            for _, row in df_raw.iterrows():
                try:
                    ...
                    to_insert.append({
                        ...
                        "montant": mt, 
                        "compte": compte_nom, 
                        "solde": st_val, # On inclut le solde
                        "categorie": cat, 
                        "sous_categorie": sub
                    })
                except: continue
            ...
            if to_insert:
                # --- CALCUL DE L'OCCURRENCE POUR GÉRER LES DOUBLONS ---
                df_to_upsert = pd.DataFrame(to_insert)
                # On groupe par tout ce qui définit une transaction et on compte
                df_to_upsert['occurrence'] = df_to_upsert.groupby(['date', 'libelle', 'montant', 'compte', 'solde']).cumcount()
